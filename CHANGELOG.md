...
### fix: OCR 字符重复修复 — 三层去重
- **OCR 参数优化**：`det_db_unclip_ratio` 从 1.8 降至 1.5，`det_db_box_thresh` 从 0.5 提至 0.6，减少重叠检测框
- **裁剪 padding 优化**：`ocr_blocks` 的 crop padding 从 5px 降至 2px，避免相邻行文字混入
- **NMS 去重**：新增 `_nms_ocr_lines()` 对重叠检测框做 IoU 去重，保留文字最长的一个
- **文字级合并**：新增 `_merge_suffix_prefix_lines()` 同行相邻 bbox 若文字前后缀重叠则合并去重
- **目录字符去重**：新增 `_dedupe_repeated_chars()` 去除中文字符连续重复（仅在 TOC 解析时应用），白名单保留"看看""好好"等合法叠词
- 修复案例：`第第一一章牙体组织` → `第一章牙体组织`，`腔组织病理学` → `口腔组织病理学`
