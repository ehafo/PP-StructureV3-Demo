...
- 保留 OCR 引擎默认参数 (`unclip=1.8`, `box_thresh=0.5`)：此前尝试调紧检测参数虽然减少重复但造成笔画复杂的字（骨/颌/组）被切掉或输出 `·` 占位符，故回滚
- 保留 `pad=5` 裁剪范围：避免边缘字符被裁切
- 修复案例：`第第一一章牙体组织` → `第一章牙体组织`，`第二二节节牙质` → `第二节牙质`
